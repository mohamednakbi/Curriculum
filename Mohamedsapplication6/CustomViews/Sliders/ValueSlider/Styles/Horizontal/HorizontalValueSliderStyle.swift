    ...
    public func makeBody(configuration: Self.Configuration) -> some View {
        ...
        return GeometryReader { geometry in
            ZStack {
                if self.options.contains(.interactiveTrack) {
                    track.gesture(DragGesture(minimumDistance: 0)
                        .onChanged { gestureValue in
                            let computedValue = valueFrom(distance: gestureValue.location.x,
                ...
                .position(x: distanceFrom(value: configuration.value.wrappedValue,
                                          availableDistance: geometry.size.width,
                                          bounds: configuration.bounds,
                                          leadingOffset: self.thumbSize.width / 2,
                                          trailingOffset: self.thumbSize.width / 2),
                    ...
                    .onChanged { gestureValue in
                        if configuration.dragOffset.wrappedValue == nil {
                            configuration.dragOffset.wrappedValue = gestureValue.startLocation
                                .x - distanceFrom(value: configuration.value.wrappedValue,
                        ...
                        let computedValue = valueFrom(distance: gestureValue.location
        ...
        .frame(minHeight: thumbInteractiveSize.height)
    }
...
public extension HorizontalValueSliderStyle where Track == DefaultHorizontalValueTrack {
    init(thumb: Thumb, thumbSize: CGSize = CGSize(width: 27, height: 27),
         ...
         options: ValueSliderOptions = .defaultOptions)
    {
        track = DefaultHorizontalValueTrack()
        self.thumb = thumb
        self.thumbSize = thumbSize
        self.thumbInteractiveSize = thumbInteractiveSize
        self.options = options
