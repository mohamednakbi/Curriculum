...
public struct VerticalValueSliderStyle<Track: View, Thumb: View>: ValueSliderStyle {
    ...
    public func makeBody(configuration: Self.Configuration) -> some View {
        let track = self.track
            ...
            .environment(\.valueTrackConfiguration,
                         ValueTrackConfiguration(bounds: configuration.bounds,
        ...
        return GeometryReader { geometry in
            ZStack {
                if self.options.contains(.interactiveTrack) {
                    track.gesture(DragGesture(minimumDistance: 0)
                        .onChanged { gestureValue in
                            let computedValue = configuration.bounds
                ...
                .position(x: geometry.size.width / 2,
                          y: geometry.size
                              .height - distanceFrom(value: configuration.value.wrappedValue,
                ...
                .gesture(DragGesture(minimumDistance: 0)
                    .onChanged { gestureValue in
                        ...
                        let computedValue = valueFrom(distance: geometry.size
                            .height -
        ...
        .frame(minWidth: thumbInteractiveSize.width)
    }
...
public extension VerticalValueSliderStyle where Track == DefaultVerticalValueTrack {
    init(thumb: Thumb, thumbSize: CGSize = CGSize(width: 27, height: 27),
         thumbInteractiveSize: CGSize = CGSize(width: 44, height: 44),
         options: ValueSliderOptions = .defaultOptions)
    {
        track = DefaultVerticalValueTrack()
        self.thumb = thumb
        self.thumbSize = thumbSize
        self.thumbInteractiveSize = thumbInteractiveSize
        self.options = options
...
public extension VerticalValueSliderStyle where Thumb == DefaultThumb {
