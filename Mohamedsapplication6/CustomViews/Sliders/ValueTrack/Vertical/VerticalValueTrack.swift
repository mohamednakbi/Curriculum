    ...
    public var body: some View {
        GeometryReader { geometry in
            self.view.accentColor(.accentColor)
                .mask(ZStack {
                    self.mask
                        .frame(height: distanceFrom(value: self.value,
...
public extension VerticalValueTrack where ValueView == DefaultVerticalValueView {
    init(mask: MaskView) {
        self.init(view: DefaultVerticalValueView(), mask: mask)
    }
}

public extension VerticalValueTrack where MaskView == Capsule {
    init(view: ValueView) {
